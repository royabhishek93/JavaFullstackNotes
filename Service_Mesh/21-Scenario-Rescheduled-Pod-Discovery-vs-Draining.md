...
## Discovery vs. Draining Timeline (Sequence Diagram)

```
Participants:
  Old    = Old Pod (user-auth-service)      K8s    = Kubernetes / Endpoints
  Mesh   = Mesh Sidecar (Envoy)              New    = New Pod (user-auth-service)
  Caller = recommendation-service

K8s    --- terminate / reschedule --------------------------------> Old
Old    --- in-flight connections abruptly cut (no draining) [self]
K8s    --- schedule + start new pod -------------------------------> New

  +--- par: outlier detection lag ------------------------------------------+
  | Mesh   --- still routing to Old (health check interval too slow) [self] |
  | Caller --- request -----------------------------------------> Mesh     |
  | Mesh   --- forward (fails - pod gone) ------------------------> Old    |
  | Mesh   --- error --------------------------------------------> Caller  |
  +---------------------------------------------------------------------- +

Mesh   --- endpoints list + health status converge (~30s) [self]
Caller --- request -----------------------------------------------> Mesh
Mesh   --- forward (success) ---------------------------------------> New
```
