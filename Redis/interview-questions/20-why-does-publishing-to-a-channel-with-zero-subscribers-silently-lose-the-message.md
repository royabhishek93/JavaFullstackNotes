...
This also surprises engineers migrating from Kafka specifically: a Kafka consumer can start reading a topic from an earlier offset and "catch up" on messages published before it connected, because Kafka persists everything to disk. Redis Pub/Sub has no equivalent — one team migrating a Kafka pipeline to Pub/Sub for cost reasons assumed a newly-deployed consumer would "catch up" on events published during its own deployment, just like it did on Kafka. Every event published during those few seconds of downtime was silently and permanently lost instead.
