...
SQLite
✓ Great for
Transactions, complex queries, strong consistency, well-defined relationships.
✕ Avoid when
Unpredictable schemas, extreme horizontal scale, giant unstructured blobs.
Example data shape
Think: spreadsheet-like tables with the same columns in every row.
USERS TABLE
+----+--------+-----------+
| ID | Name   | City      |
+----+--------+-----------+
...
Amazon DocumentDB
✓ Great for
Evolving schemas, content & catalogs, nesting data you read together.
✕ Avoid when
Heavy multi-entity joins and strict multi-record transactions.
Example data shape
Think: one self-contained JSON document per object.
{
  "userId": 101,
  "name": "Bunny",
  "skills": ["Java", "Spring"],
...
Riak
✓ Great for
Caching, sessions, leaderboards, rate limits, feature flags.
✕ Avoid when
Querying by anything other than the key; complex relationships.
Example data shape
Think: a HashMap — provide the key and get the value.
session:user:101  →  { "loggedIn": true }
otp:9876543210     →  438921
theme:user:101     →  dark
...
Google Bigtable
✓ Great for
High write throughput, predictable key-based queries, IoT, event history, messaging and telemetry.
✕ Avoid when
Ad-hoc queries, joins, strong transactional consistency.
Example data shape
Think: a partition key groups a potentially very wide, ordered set of cells for fast known queries.
USER 101
  name    = Bunny
  city    = Kolkata
  premium = true
...
TigerGraph
✓ Great for
Social networks, recommendations, fraud rings, knowledge graphs.
✕ Avoid when
Simple tabular workloads or bulk analytical scans.
Example data shape
Think: entities are nodes; connections are named relationships.
(Bunny) ──FRIEND_OF──> (Rahul)
   │
 PURCHASED
   ↓
(iPhone)
...
Meilisearch
✓ Great for
Full-text search, relevance ranking, log & observability analytics.
✕ Avoid when
Being your source of truth for transactional writes.
Example data shape
Think: words point to matching documents, enabling ranked text search.
SEARCH: "java interview"

1. Java Interview Guide
2. Spring Boot Questions
3. Java Collections Explained
...
QuestDB
✓ Great for
Metrics, monitoring, sensor & financial ticks, IoT telemetry.
✕ Avoid when
Relational, frequently-updated, non-time-ordered data.
Example data shape
Think: every measurement is attached to a timestamp.
TIME      CPU_USAGE
10:00     20%
10:01     22%
10:02     18%
...
Chroma
✓ Great for
Semantic search, RAG for LLMs, recommendations, image similarity.
✕ Avoid when
Exact-match lookups and traditional transactional workloads.
Example data shape
Think: content becomes a numeric vector and is searched by meaning.
"cute puppy" → [0.23, 0.91, 0.77, ...]
...
VoltDB
✓ Great for
Global apps needing transactions AND massive horizontal scale.
✕ Avoid when
Small apps where a single Postgres node is plenty.
Example data shape
Think: relational tables distributed across multiple database nodes.
APPLICATION
     ↓
DISTRIBUTED SQL
 ├── Node 1: India
...
Same SQL interface + ACID transactions
...
Databricks
✓ Great for
BI dashboards, aggregations, reporting over huge historical data.
✕ Avoid when
High-frequency single-row reads/writes (that's OLTP's job).
Example data shape
Think: years of historical data scanned for totals, trends, and reports.
YEAR    ORDERS      REVENUE
2024    5 million   ₹120 crore
2025    8 million   ₹190 crore
2026   12 million   ₹280 crore

Query: SUM(revenue) GROUP BY year
...
A strong choice for transactional web applications when the team, tooling, hosting platform or packaged software is already standardized on MySQL.

Mature InnoDB transaction and replication ecosystem
Common across traditional web stacks and hosted platforms
Operational familiarity is widely available
Good when requirements are conventional and expertise already exists
PostgreSQL
Feature-rich default
A strong general-purpose default when you expect advanced SQL, rich data types, extensibility, geospatial work or mixed relational and JSON workloads.

Advanced SQL, JSONB, arrays and range types
PostGIS for geospatial applications
Rich extension ecosystem and complex integrity rules
Good when future query complexity is uncertain
Decision shortcut: Both can run serious transactional systems. Prefer PostgreSQL for richer built-in capabilities and extensibility; prefer MySQL when compatibility, existing expertise, hosting support or ecosystem standardization dominates.
💡 Think of it like this: Imagine a clothing store needing customers, orders, inventory and billing. MySQL is more than enough. Now imagine a food-delivery or maps platform needing geospatial queries, JSON data and complex reporting. PostgreSQL becomes the more natural fit.
Document
MongoDB vs CouchDB
›
