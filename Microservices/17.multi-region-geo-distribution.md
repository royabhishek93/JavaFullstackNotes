...
```
   US (Primary)              EU (Standby)             Asia (Standby)
  ┌───────────┐   replicate  ┌───────────┐  replicate  ┌───────────┐
  │ App + DB  │ ───────────▶ │ DB (read) │ ───────────▶│ DB (read) │
  │ (R + W)   │              │  (R only) │             │  (R only) │
  └───────────┘              └───────────┘             └───────────┘
        ▲
        │  ALL writes, from every region, go here
   writes from EU/Asia users travel cross-region (higher latency)
```
...
```
  ...
  │ (R + W)   │              │  (R + W)  │             │  (R + W)  │
  └───────────┘              └───────────┘             └───────────┘
   local writes = low latency, but concurrent writes can conflict
   → needs version vectors / CRDTs / last-write-wins to reconcile
```
...
```
                         DNS query: shop.example.com
                                    │
                                    ▼
                    ┌───────────────────────────────┐
                    │   Geo DNS / Anycast resolver   │
                    │  (looks at requester's IP geo) │
                    └───────────────────────────────┘
                       │            │            │
           user in US  │  user in EU│  user in Asia│
                       ▼            ▼            ▼
                 us.shop.com   eu.shop.com   asia.shop.com
                       │            │            │
                       ▼            ▼            ▼
                  Region: US    Region: EU    Region: Asia
                (lowest latency for each user, no cross-ocean hop)
```
