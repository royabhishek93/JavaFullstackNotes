...
├──────────────────────┼─────────────────────────────────────────────────────────────────────┤
│ Redis Cache          │ WHY: 115K RPS read load, but Zipf distribution means ~10-20GB       │
│                      │ of "hot" URLs drives 80% of traffic. Redis LRU cache absorbs        │
│                      │ this load. Sub-millisecond response time. Cache-aside keeps         │
│                      │ Redis as a pure acceleration layer, not a source of truth.          │
│                      │ WHY NOT Memcached: Redis supports TTL-per-key natively, which       │
│                      │ we need for URL expiry. Redis also has richer data structures       │
│                      │ for future features (counters, sorted sets for analytics).          │
├──────────────────────┼─────────────────────────────────────────────────────────────────────┤
...
{
  "longUrl": "https://www.example.com/very/long/path?ref=newsletter&utm_source=email",
  "customAlias": "my-link",
  "expiresIn": 2592000
}
...
// Response (201 Created):
{
  ...
  "longUrl": "https://www.example.com/very/long/path?ref=newsletter",
...
{
  "error": "ALIAS_TAKEN",
  "message": "Custom alias 'my-link' is already in use. Please choose another."
}
```
...
Location: https://www.example.com/very/long/path?ref=newsletter

// Response (expired): HTTP 410 Gone
{
  "error": "URL_EXPIRED",
...
## STEP 5c — Alternative HLD: Redis Counter Approach (from image)

> This is what the image shows as the "Counter" approach. The guide recommends Zookeeper
> range allocation (Option C in STEP 7) but interviewers may ask about this variant too.

```
SERVER WORKLOAD SPLIT:
  ┌────────────────────────────────────────┐
  │  Server                                │
  │  ─ encryption (20%)  ← URL creation   │
  │  ─ decryption (80%)  ← URL redirect   │
  └────────────────────────────────────────┘
  Takeaway: system is READ-HEAVY (80% redirects).
  Every design decision should optimise the read path.

ARCHITECTURE (client → LB → multiple servers → Redis counter → DB):
  ...
  │  301 / 302-redirect  ← type depends on analytics requirement     │
  └──────┬───────────────────────────────────────────────────────────┘
         │
         ├──────────────────┬────────────────────┐
         ▼                  ▼                    ▼
...
LATENCY BREAKDOWN (from image):
  ┌──────────────────────────────────────────────────────────┐
  │  Step               │ Latency                            │
  ├─────────────────────┼────────────────────────────────────┤
  │  Encryption / hash  │  1 ms                              │
  │  Server → Redis     │  2–3 ms  (counter increment)       │
  │  Server → DB        │  5 ms    (write or read)           │
  │  Total round trip   │  ~16 ms + 16 ms (client ↔ server) │
  └──────────────────────────────────────────────────────────┘

WHY Zookeeper RANGE is better than Redis INCR counter:
  Redis INCR: every URL creation needs a Redis round-trip (2-3ms).
              Redis becomes a bottleneck at 1,157 writes/sec × multiple servers.
  Zookeeper range: server fetches 1M IDs in one call → stores in memory.
                   next 1M creates: ZERO network calls. 0ms overhead.
  → Prefer Zookeeper range allocation for production (STEP 7, Option C).
```
...
## STEP 5b — SEQUENCE DIAGRAM: URL Shortening + Redirect Flow

### CREATE SHORT URL:
```
  Client          API Gateway      URL Service       Zookeeper        MySQL            Redis
    │                  │                │                  │               │               │
    │ POST /shorten    │                │                  │               │               │
    │ {longUrl,alias?} │                │                  │               │               │
    │─────────────────▶│                │                  │               │               │
    ...
    │                  │                │  [nextRange]     │               │               │
    │                  │                │                  │               │               │
    │                  │                │ Base62(counter)=shortCode        │               │
    │                  │                │                  │               │               │
    │                  │                │ INSERT url_mappings               │               │
    │                  │                │─────────────────────────────────▶│               │
    │                  │                │◀─────────────────────────────────│               │
    │                  │                │       [OK]       │               │               │
    │                  │                │ SET url:{shortCode} longUrl EX ttl               │
    │                  │                │─────────────────────────────────────────────────▶│
    │                  │ {shortCode,    │                  │               │               │
    │◀─────────────────│  shortUrl}     │                  │               │               │
```
...
```
┌─────────────────────────────────────────────────────────────────┐
│                  TINYURL — ENTITY RELATIONSHIP                   │
└─────────────────────────────────────────────────────────────────┘

┌──────────────────┐          ┌──────────────────────┐
│      users       │          │    url_mappings      │
├──────────────────┤          ├──────────────────────┤
│ PK user_id BIGINT│◄────────│ FK user_id BIGINT    │
│    email TEXT    │  1   N  │ PK short_code VARCHAR │
│    plan ENUM     │         │    long_url TEXT      │
│    created_at TS │         │    custom_alias BOOL  │
└──────────────────┘         │    expires_at TS      │
                             │    is_active BOOL     │
                             └──────────┬────────────┘
                                        │ 1
...
─────────────────────────────────────────────────
  ...
  PROBLEM: Single counter is a global bottleneck.
    Every write must serialize through one place. At 1,157 writes/sec this is manageable
    but becomes a single point of failure and scaling ceiling.
...
────────────────────────────────────────────────────────
  ...
  BIT STRUCTURE (from image):
  ┌──────────────────────────────────────────────────────────────────────┐
  │  [1 bit unused][41 bits timestamp][10 bits workerID][12 bits seq]    │
  │                                                                      │
  │  41-bit timestamp math (from image):                                 │
  │    2^41 / 365 / 24 / 3600 / 1000 = 69.73 years                      │
  │    → IDs won't overflow for ~70 years from custom epoch              │
  │                                                                      │
  │  12-bit sequence math (from image):                                  │
  │    2^12 × 1000 = 4,096,000 IDs per second per worker                │
  │    (4096 IDs per millisecond × 1000 ms/sec)                         │
  │                                                                      │
  │  10-bit workerID: 2^10 = 1024 workers (servers)                     │
  │  WorkerIDs assigned: workerID 123, workerID 234, workerID 345...     │
  └──────────────────────────────────────────────────────────────────────┘

  Example:
    Server 123, timestamp ms=189432045123, sequence=0
    ID = (189432045123 << 22) | (123 << 12) | 0
    Base62 encode → 10-char short code (longer than counter's 7 chars)
...
─────────────────────────────────────────────
  ...
  WHAT IS ZOOKEEPER? (from image):
  ┌────────────────────────────────────────────────────────────────────┐
  │  ZooKeeper is a distributed, open-source coordination service for  │
  │  distributed applications, providing centralized services for:     │
  │  configuration management, naming, synchronization, and group      │
  │  services — making it easier to manage large clusters.             │
  │  Used for: distributed synchronization, distributed locking,       │
  │  configuration info, naming, group service.                        │
  │                                                                    │
  │  ZOOKEEPER TREE HIERARCHY:                                         │
  │                    /                                               │
  │                   / \                                              │
  │               /app1   /app2                                        │
  │               /  |  \                                              │
  │         /ip_1 /ip_2 /ip_3   ← each encryption server node         │
  │                                                                    │
  │  For URL shortener: /counters/url_counter node stores              │
  │  the current global counter. Atomic setData() ensures             │
  │  no two servers ever get the same counter range.                   │
  └────────────────────────────────────────────────────────────────────┘
