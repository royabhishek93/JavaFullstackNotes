...
### Rider APIs (Missing from above)
```
GET  /v1/api/drivers/nearby        → ?lat=X&lng=Y&radius=Z&vehicle_type=sedan → [{driver_id, name, lat, lon, eta_min, rating, vehicle_model}]
```

### Driver APIs (Missing from above)
```
PATCH /v1/api/driver/{driver_id}/status  → {status: 'AVAILABLE'/'OFFLINE', lat, lon} → updated driver status + geospatial pool updated
```

### Device Registration (Shared)
```
POST /v1/api/devices               → {user_id, device_token, platform: 'ios'/'android'} → device registered for push notifications
```

> **WHY GET /drivers/nearby?** Functional requirement #4 explicitly states "See nearby available drivers on map in real-time." The rider app calls this on map load to render driver pin markers before the user even taps "Request." Under the hood it maps to `GEORADIUS drivers:available {lon} {lat} {radius}km WITHDIST ASC` in Redis (~10ms for 1M drivers). Without this endpoint the live map of available cars is impossible to populate.

> **WHY PATCH /driver/{driver_id}/status?** Functional requirement #8 states "Drivers update status (available/busy/offline)." This is the explicit go-online / go-offline call made when a driver opens or closes the app. It is described in detail in Step 11 (updating Redis `GEOADD`/`GEOREM`, DB `UPDATE drivers SET status`, and setting the heartbeat TTL) but is absent from the formal Section 4 API block — a gap interviewers will notice when they ask "how does a driver go online?"

> **WHY POST /devices?** The entire push notification system (Step 10 — FCM/APN) depends on storing a device token per user at login or app-install. Without registering the token, the Notification Service has no address to send ride-matched, driver-arrived, or payment-completed push alerts. This call is spelled out in Step 10 but missing from the formal API design section.
...
> **WHY ETA EXISTS? (Beginner Explanation)**
> ETA is like a GPS app telling you "arrive in 12 minutes." The Ride Service doesn't guess —
> it calls Google Maps Distance Matrix API with pickup and drop coordinates to get real drive
> time and distance. That same ETA is shown to the rider before they even book, so they know
> what they're signing up for. Without it, hailing an Uber would feel like calling a taxi and
> being told "it'll come eventually." The eta_min is also passed to the notification ("Driver
> arriving in 5 min") and recalculated live during tracking as the driver moves through traffic.

---

### Step 2: Ride Request & Driver Matching

> **WHY GEOSPATIAL SEARCH FOR DRIVER MATCHING? (Beginner Explanation)**
> Finding the nearest driver is like asking "which food stall is closest to me in a crowded
> market?" You can't compare every driver's lat/lng one by one — at 1 million drivers that
> is a full table scan too slow for a 1-second SLA. Instead, Redis stores all driver
> locations in a geospatial index (like a pre-sorted city grid), and GEORADIUS queries it
> in ~10ms. Results come back sorted by distance, so the matching service always tries the
> closest driver first, expanding the search radius if none are found nearby.
...
> **WHY KAFKA FOR RIDE EVENTS? (Beginner Explanation)**
> Kafka is like the order ticket printer in a restaurant kitchen — the waiter drops the
> ticket and walks away; the chef picks it up when ready. When a rider requests a ride,
> the Ride Service publishes a "ride.requested" event to Kafka and returns immediately —
> it does not wait for matching to finish. The Driver Matching Service consumes that event
> separately, at its own pace. This decoupling means a slow matching step never freezes
> the rider's app, the event is durably stored even if matching is temporarily down, and
> multiple consumers (matching, analytics, notifications) can all read the same event
> independently without the Ride Service knowing or caring who's listening.
...
> **WHY ZOOKEEPER DISTRIBUTED LOCKING? (Beginner Explanation)**
> Imagine 3 taxi dispatchers simultaneously calling the same driver for 3 different passengers.
> Without coordination, the driver gets triple-assigned. Zookeeper acts as a shared whiteboard
> with a strict rule: only one dispatcher can write on it at a time. The first to write wins
> the driver; the others see it is taken and immediately try the next closest driver instead.
> The "ephemeral node" trick is the safety net — if a dispatcher's computer crashes mid-booking,
> the lock auto-disappears after 30 seconds rather than staying locked forever. Redis SETNX
> could also lock, but Zookeeper is stronger: it guarantees consistency across all nodes and
> handles server crashes automatically via session timeouts.
...
> **WHY WEBSOCKET INSTEAD OF HTTP POLLING? (Beginner Explanation)**
> HTTP polling is like calling a friend every 3 seconds to ask "where are you now?" — each
> call re-dials, waits for them to pick up, exchanges pleasantries, then hangs up. WebSocket
> is like leaving the call open the entire trip — location pings flow the instant the driver
> moves, with no re-dial overhead. At 100K active trips, polling would flood the servers with
> 33K HTTP requests per second, each carrying full headers. WebSocket keeps one persistent
> connection open per driver and sends only small coordinate packets, reducing bandwidth by 10×
> and cutting latency from 1–3 seconds down to under 100ms.
>
> **WHY REDIS FOR DRIVER LOCATION STORAGE? (Beginner Explanation)**
> Driver locations change every 3–5 seconds — that is 100K+ writes per second across all
> active drivers. PostgreSQL is like a filing cabinet: excellent for permanent records, but
> slow and expensive for constant rewrites at this volume. Redis is the sticky note on the
> fridge — in-memory, blindingly fast, and with TTL 60s it auto-discards stale locations
> the moment a driver stops sending updates. PostgreSQL keeps the permanent location history
> (via Kafka → Trip Update Consumer); Redis answers the live question "where is this driver
> right now?" in under 1ms.
...
> **WHY SURGE PRICING? (Beginner Explanation)**
> Think of surge like taxi prices on New Year's Eve — more people want rides than drivers
> are available, so prices rise to attract more drivers onto the road. The system divides
> the city into 5km×5km grid squares and measures demand vs supply every 60 seconds per
> square. If 10 riders are waiting but only 2 drivers are available (ratio 5×), the fare
> doubles. The smoothing formula (70% old + 30% new) prevents prices from spiking wildly —
> a sudden one-minute calculation spike won't shock a rider with an instant 3× fare jump.
> The rider must explicitly tap "Accept higher fare" — the system never silently charges surge.
>
> **WHY GEOHASH FOR SURGE CELLS? (Beginner Explanation)**
> Geohash converts any (latitude, longitude) pair into a short string like "dr5ru" — and
> nearby locations share the same prefix, like houses on the same street sharing a ZIP code.
> This lets the surge calculator say "give me all pending rides in this cell" with a single
> Redis key lookup (`surge_multiplier:dr5ru`) instead of running expensive distance math
> across thousands of lat/lng pairs. It is how Uber groups demand and supply into meaningful
> geographic buckets — a 5km×5km square is big enough to have multiple drivers but small
> enough that rain in one area doesn't inflate prices across an entire city.
...
> **WHY A TRIP STATE MACHINE? (Beginner Explanation)**
> A trip state machine is like the stages of a restaurant order: placed → confirmed →
> being prepared → ready → delivered → paid. Each stage only moves forward (you cannot
> un-start a trip), and certain actions are only valid in certain states — a driver cannot
> "complete" a trip that is still PENDING, and a rider cannot rate before the trip ends.
> Each state transition also triggers different downstream actions: MATCHED sends driver
> details to the rider; IN_PROGRESS starts the fare meter; COMPLETED triggers payment
> processing and the rating prompt. The ENUM column in the DB enforces these rules at
> the data layer, so even a buggy client cannot put a trip into an impossible state.
...
**Why it matters here:** Driver sends GPS location every 3s (client→server) AND receives dispatch events (server→client). Both directions active simultaneously → WebSocket. SSE is one-way only and would require a separate upload channel.
**Deep dive:** `../../WebSocket_vs_SSE_vs_Long_Polling.md`

### CAP Theorem
**Why it matters here:** UBER is AP for driver location tracking — stale driver position by 3-5 seconds is acceptable. Rider sees approximate location. Availability over perfect consistency for real-time tracking.
**Deep dive:** `../../CAP_Theorem_Applied_What_Actually_Breaks.md`

### Heartbeat Detection
**Why it matters here:** Detecting driver app disconnects (phone tunnel, no signal). 3 missed WebSocket pings → driver marked unavailable → no new dispatch. False positive timeout too short means a driver loses rides from a brief tunnel entry.
...
**Why this system uses it:** Driver location storage and "find nearby drivers" query are the core of UBER's matching service. Redis GEORADIUS (which uses geohash internally) stores 1M active driver locations. `GEORADIUS user:location 77.5946 12.9716 5 km ASC COUNT 10` returns 10 nearest drivers within 5km in milliseconds. Driver location updates every 3s: `GEOADD drivers {lng} {lat} driver:{id}` — Redis handles 333K writes/sec across a cluster. At city-level sharding: shard by geohash prefix (e.g., all "tdr" prefix = Bangalore shard).

### [AWS API Gateway — REST vs HTTP vs WebSocket](../../../aws/22.api-gateway-rest-http-websocket-architect-interview.md)
**Why this system uses it:** Driver location updates need persistent connections — WebSocket API. Each driver app maintains a WebSocket connection (connectionId). The location service receives driver coordinates and broadcasts to nearby rider apps via connectionId lookup. HTTP API (v2) for REST endpoints (get ride status, search drivers) — 71% cheaper and lower latency than REST API, perfect for high-QPS location lookups.

### [Kinesis vs MSK Kafka vs SQS — Streaming Decision](../../../aws/23.kinesis-vs-msk-kafka-vs-sqs-streaming-decision.md)
**Why this system uses it:** Driver location events (1 update/second per driver, 500K active drivers = 500K events/sec) use Kinesis Data Streams with `partitionKey=driverId` — ensures all events for one driver land on the same shard (ordered). Enhanced Fan-Out: location service consumer + fraud detection consumer each get dedicated 2MB/s per shard without starving each other. Ride matching uses SQS FIFO (`MessageGroupId=rideId`) for ordered state transitions.

### [Route53 Advanced Routing Policies](../../../aws/29.route53-routing-policies-dns-failover-architect-interview.md)
**Why this system uses it:** Latency-based routing for UBER's global presence — users in India routed to ap-south-1, US users to us-east-1, ensuring driver matching happens in the closest region. Failover policy: primary region health check monitors the ride-matching service; if unhealthy, Route53 shifts traffic to standby region within 90 seconds (TTL=60 + 3×10s health checks). Weighted routing for canary deployments of new matching algorithm — 5% traffic to new version before full rollout.
...
**Why this system uses it:** Active-active architecture for UBER — drivers in Mumbai should be matched with riders in Mumbai without crossing the ocean. DynamoDB Global Tables for driver location and availability (LWW acceptable — latest location wins). Aurora Global Database for trip history and billing records (compliance + consistency needed). Global Accelerator routes dynamic ride-matching API traffic through AWS backbone — 40% lower latency vs public internet for real-time location updates.
