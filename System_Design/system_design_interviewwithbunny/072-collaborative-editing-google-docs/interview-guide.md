...
WHY WEBSOCKET EXISTS? (Beginner Explanation)
  Imagine you're on the phone with a friend — the line stays open so either of you can
  talk any time. That's WebSocket. HTTP is the opposite: like sending a letter and waiting
  for a reply — you have to keep sending new letters just to check if anything changed.
  For Google Docs with 100 people typing at once, HTTP polling means 100 people × 1
  request/sec = 100 req/sec per doc, times millions of docs = the server drowns.
  WebSocket opens ONE persistent connection per user; the server pushes updates the
  instant they happen — no polling, no overhead.
  Without it: constant "Are we there yet?" requests. With it: the server taps your
  shoulder only when something actually changed.
...
### REST — Additional Endpoints
```
GET    /v1/api/docs                              → list documents owned by or shared with the authenticated user
DELETE /v1/api/docs/{docId}                      → soft-delete document (owner only)
POST   /v1/api/docs/{docId}/share                → grant or revoke access { user_id, role: 'view'|'edit'|'comment' }
GET    /v1/api/docs/{docId}/collaborators         → list all users with access and their current roles
GET    /v1/api/docs/{docId}/export?format=pdf     → export document (pdf | docx | txt), returns pre-signed S3 URL
```

> **WHY GET /v1/api/docs?** Every "My Documents" dashboard needs a paginated list of docs for the authenticated user. Without it there is no entry point — users would need to know the exact doc_id to navigate anywhere. Filters on owner_id or shared-with; supports cursor-based pagination for large collections.

> **WHY DELETE /v1/api/docs/{docId}?** Functional requirements explicitly include delete. Soft-delete (set `deleted_at`, purge after a grace period) prevents accidental data loss. Only the owner or an admin can invoke it. Without it storage grows unboundedly and there is no product-level "trash" feature.

> **WHY POST /v1/api/docs/{docId}/share?** Sharing is a named functional requirement (View / Edit / Comment permissions). This endpoint writes to the `permissions` JSONB on the `documents` table. It is the API surface behind every product "Share" button and permission-change flow.

> **WHY GET /v1/api/docs/{docId}/collaborators?** The share dialog must display who currently has access. Derived from the `permissions` JSONB column. Interviewers commonly ask "how does the share dialog populate?" — this endpoint is the direct answer.

> **WHY GET /v1/api/docs/{docId}/export?format=pdf?** Users need to take documents outside the platform. Triggers an async export job: latest snapshot → render PDF/DOCX → upload to S3 → return a pre-signed URL. Demonstrates awareness of async fan-out for expensive CPU-bound operations.

---

WHY REDIS FOR ACTIVE DOCUMENT SESSIONS EXISTS? (Beginner Explanation)
  Redis is the sticky note on your fridge — PostgreSQL is the filing cabinet in the
  basement. When 50 people are editing the same Google Doc right now, every keystroke
  needs the current document content in under 1ms. Fetching from S3 (basement filing
  cabinet) takes 80–200ms; replaying thousands of ops from PostgreSQL takes seconds.
  Redis keeps the "live" copy of the document in RAM with a 30-minute expiry — it's
  thrown away once everyone leaves (no one needs a sticky note after the meeting).
  Without Redis: every edit would trigger a round-trip to S3 + op replay → 200ms lag
  per keystroke × 100 editors = completely unusable. With Redis: 1ms read, 90% of
  requests never touch the database at all.
...
WHY OPERATIONAL TRANSFORMATION (OT) EXISTS? (Beginner Explanation)
  Picture two people editing the same whiteboard at the same time over video call — each
  sees the board as it was when they started. Alice writes "Hello" at position 0; Bob
  writes "World" at position 0 simultaneously. If you just apply both blindly, one of
  them gets overwritten or lands in the wrong place.
  OT says: "Before I apply Bob's operation, I'll adjust it to account for what Alice
  already did." Alice inserted 5 characters before Bob's position, so Bob's position
  shifts by +5. Now both edits land correctly and everyone's document converges to the
  same text — guaranteed.
  The alternative (last-write-wins) would randomly discard one person's work. OT makes
  concurrent edits commute safely by having a central server assign a definitive order
  and recompute positions accordingly.
...
WHY CRDT (CONFLICT-FREE REPLICATED DATA TYPE) EXISTS? (Beginner Explanation)
  OT needs a referee (central server) to call the order of edits. CRDT says: what if we
  designed the data so there's never a conflict in the first place?
  Think of a set of sticky notes, each stamped with a globally unique sticker (site_id +
  sequence). No two notes are ever "the same position" — every character is identified
  by its unique ID, not its index. If Alice deletes note {1,1} while Bob inserts a note
  after {1,1} while both are offline, when they reconnect it doesn't matter which op
  arrives first — the result is the same either way, because the operations don't step
  on each other's toes.
  The cost: every character carries extra metadata (its unique ID), and deleted characters
  leave "tombstones" (invisible placeholders) so anchors stay valid. Memory is higher, but
  you get true peer-to-peer sync with zero server coordination — ideal for offline-first
  apps like Figma or Automerge.
...
WHY CURSOR PRESENCE EXISTS? (Beginner Explanation)
  In a physical meeting room, you can see where everyone else is pointing on the
  whiteboard — that awareness prevents two people from writing in the same spot at the
  same time. Cursor presence gives you that same spatial awareness in a digital doc: you
  see Alice's blue cursor at paragraph 3, Bob's green cursor at the title, so you
  naturally avoid each other.
  Implementation: each cursor move fires a WebSocket event with the user's position and
  selection range. It's throttled to every 100ms (no need to send every single pixel
  move), stored in Redis as a temporary hash (expires in 5 minutes), and broadcast to
  all other editors in that doc. When Bob edits text before Alice's cursor, her cursor
  position is automatically shifted by the OT transform so it stays pointing at the
  right word.
  Critically: cursors are NEVER saved to the database — they exist only while you're
  online and disappear the moment you close the tab.
...
WHY SNAPSHOTS AND VERSIONING EXIST? (Beginner Explanation)
  Imagine a Word document that only saves a record of every single keystroke you ever
  made, from the very beginning. To open it, your computer has to replay every keystroke
  in order — after a year of edits, that would take minutes. Snapshots are periodic
  "photographs" of the document at a point in time, compressed and stored in S3.
  Instead of replaying 10,000 operations, you load the latest photo (50 ops ago) and
  replay only the last 50 keystrokes — milliseconds instead of seconds.
  Version history is just a list of those photos with timestamps and author info. When
  you click "See version from Tuesday," the system fetches that S3 photo and shows it
  read-only. Rollback creates a NEW photo that happens to look like an old one — history
  is never mutated, only appended to.

WHY DIFF AND PATCH EXISTS? (Beginner Explanation)
  When you type a single letter into a 100,000-character document, the system does NOT
  send the entire document over the network — that would be like emailing someone the
  whole book every time you correct a typo. Instead it sends a diff (the change): "insert
  'R' at position 29." That tiny operation is a patch the receiver applies to their own
  copy. This is why the Operation entity contains just type + position + content rather
  than the full document. Every user's client holds their own local copy and patches it
  with incoming ops. The result: a 100KB document edit costs ~50 bytes on the wire, not
  100KB. Without diffs: Google Docs would be unusable on anything but gigabit broadband.
...
WHY OFFLINE EDITING AND SYNC EXISTS? (Beginner Explanation)
  Your laptop doesn't stop being useful the moment the WiFi drops. Offline editing lets
  you keep typing into a Google Doc on the plane — the browser stores every operation
  you make in IndexedDB (like a local notepad), tagged with the last version number you
  were synced to.
  When the network comes back, the client acts like someone who just returned from
  vacation: "I was on version 42 when I left — what did I miss?" The server sends all
  ops from version 43 onward. The client applies those server ops first (they happened
  before your offline edits, so you adjust to them), then OT-transforms your queued
  local ops to account for the changes that happened while you were away, and sends the
  adjusted batch up to the server. Most of the time this is seamless. If the changes
  are irreconcilable (two people rewrote the same paragraph), a merge modal pops up —
  same as a Git merge conflict, but friendlier.
...
WHY OPTIMISTIC CONCURRENCY (NOT LOCKING) EXISTS? (Beginner Explanation)
  Pessimistic locking is like a library that only lets one person check out a book at a
  time — everyone else waits outside. That works for a rare, single-copy book, but
  imagine locking a Google Doc every time someone clicked into it. Alice locks the doc,
  goes for coffee, and 49 other people stare at a "Document locked by Alice" banner.
  Collaborative editing is ruined.
  Optimistic concurrency says: "Everyone edit freely — we'll reconcile any overlaps
  after the fact using OT." The odds of two people typing in the exact same character
  position at the same millisecond are tiny, and even when it happens, OT resolves it
  cleanly. No waiting, no blocking, no frustration.
  Pessimistic locks are still useful at the section level for high-stakes documents
  (legal contracts, financial tables) where even a small conflict is unacceptable — but
...
**Why it matters here:** Real-time collaborative editing requires bidirectional communication — user types characters (client→server) AND receives other collaborators' edits (server→client). WebSocket is the only option. SSE is one-way. Long polling has too much latency for sub-100ms collaborative editing.
**Deep dive:** `../../WebSocket_vs_SSE_vs_Long_Polling.md`

### Vector Clocks
**Why it matters here:** Concurrent edit conflict resolution — Alice and Bob both edit the same paragraph simultaneously. Each operation is tagged with a vector clock. Concurrent operations (neither happened-before the other) are merged deterministically using operational transforms. This is the mathematical foundation of Google Docs.
**Deep dive:** `../../Vector_Clocks_Write_Conflict_Detection.md`

### Content-Addressable Storage
**Why it matters here:** Document version history. Each version stores a list of 4MB block hashes. Unchanged blocks are shared across versions. 100 versions of a 10MB document cost ~10MB of storage (shared blocks), not 1GB (100 full copies). Version diff: compare block hash lists.
**Deep dive:** `../../Content_Addressable_Storage_Deduplication.md`

### CAP Theorem
**Why it matters here:** AP — during partition, local edits continue (optimistic: assume they'll merge). When partition heals, operational transforms merge concurrent edits. Availability (keep editing) > consistency (waiting for lock). This is why Google Docs works offline and syncs when you reconnect.
