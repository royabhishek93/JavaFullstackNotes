...
**Why it matters here:** SMTP server rejection (4xx transient error = "try later"). Email standard RFC 5321 mandates retry with exponential backoff: 5min, 10min, 20min, 40min... up to 24 hours. Without backoff: retry storms overwhelm recipient servers and get your sending domain blacklisted.
**Deep dive:** `../../Retry_Exponential_Backoff_Jitter.md`

### Object vs Block vs File Storage
**Why it matters here:** Email attachments (up to 25MB each, binary) → S3. Email metadata (headers, To/From, subject, folder, read/unread, search index) → PostgreSQL on EBS. Storing attachment bytes in DB = buffer pool contamination + backup disaster.
**Deep dive:** `../../Object_vs_Block_vs_File_Storage_S3_EBS_EFS.md`

### CAP Theorem
**Why it matters here:** Email is AP — during partition, accept emails for sending and queue them. Don't reject incoming emails because a replica is unreachable. At-least-once delivery: some emails may be delivered twice (idempotent deduplication by Message-ID header at receiver). Availability of email acceptance > partition-time consistency.
...
**Why this system uses it:** Email deduplication: "has this message-ID already been delivered to this mailbox?" prevents duplicate delivery when a message is retried. With billions of delivered messages, a Bloom filter checks in O(1) with near-zero memory overhead. False positive (1% rate): email not delivered despite being new → acceptable (retry resolves it). False negative: impossible by design (Bloom filter never misses existing members). HyperLogLog for "unique senders per day" — spam detection metric: sudden spike in unique senders from one IP = spam campaign indicator.

### [Inverted Index — How Elasticsearch Works](../../Inverted_Index_How_Elasticsearch_Works.md)
**Why this system uses it:** Gmail-style search — "find emails containing 'invoice' from 'amazon@amazon.com' in January 2024" — is an inverted index query at massive scale. Every email body + subject + headers indexed in Elasticsearch. Multi-field query: `{ "bool": { "must": [{"match": {"body": "invoice"}}, {"term": {"from": "amazon@amazon.com"}}], "filter": [{"range": {"date": {"gte": "2024-01-01", "lte": "2024-01-31"}}}] } }`. 10 billion emails indexed per user account at Google scale — Elasticsearch shards per user_id prefix.

### [BM25 vs Vector Search](../../BM25_vs_Vector_Search_Semantic_Similarity.md)
**Why this system uses it:** Gmail-style search combines BM25 (keyword matching for sender, subject, exact phrases like "order confirmation") with vector search (semantic queries like "find emails about the budget meeting" — even if they don't contain the word "budget"). Hybrid search: BM25 for structured metadata fields (From, To, Subject, Date), vector search for email body semantic meaning. The vast majority of email searches are keyword-based — BM25 handles them efficiently. Vector adds value for natural-language queries and when users remember the topic but not the exact wording.

### [Elasticsearch vs PostgreSQL Full-Text](../../Elasticsearch_vs_PostgreSQL_Full_Text_Search.md)
**Why this system uses it:** Email search at Gmail scale (15B emails per user account × 1.5B users) requires Elasticsearch — PostgreSQL FTS cannot handle this volume with sub-second query latency. Each user's emails are sharded by `user_id` hash across ES clusters. PostgreSQL stores email metadata (headers, folder assignments, read/unread) for ACID operations; Elasticsearch indexes the body for search. CDC (Debezium) keeps ES in sync: new email arrives → PostgreSQL write → WAL → Debezium → ES index update, searchable within 1 second.

### [WAF + Shield + GuardDuty — Security](../../../aws/27.waf-shield-guardduty-security-architect-interview.md)
**Why this system uses it:** Email platforms are high-value attack targets. WAF with AWSManagedRulesCommonRuleSet blocks SQLi/XSS in email composition API. Rate-based WAF rule: block IPs sending > 100 email API requests per 5 minutes (spam prevention). Shield Standard (free) protects against volumetric DDoS on SMTP ingestion. GuardDuty detects unusual patterns: account suddenly sending 10,000 emails/hour (compromised account, spam campaign) — HIGH finding triggers Lambda to suspend the account and notify security team. CRITICAL gotcha: WAF must be in us-east-1 if protecting a CloudFront distribution for the email web client.

### [S3 Data Lake + Athena — Email Analytics](../../../aws/26.s3-athena-data-lake-lifecycle-architect-interview.md)
**Why this system uses it:** Email delivery events (sent, delivered, bounced, spam-complained) → S3 Parquet partitioned by date/domain. Athena: "which sending domains have >5% bounce rate this week?" for deliverability monitoring. S3 lifecycle for email body storage: Standard (0-90 days) → Glacier Instant (90 days–7 years, compliance retention). Email attachments: S3 Standard → IA after 90 days → Glacier Deep Archive after 1 year. Athena provides $0.03/query analytics vs building a separate analytics DB.

### [KMS Envelope Encryption + Secrets Manager](../../../aws/28.kms-envelope-encryption-secrets-manager-architect-interview.md)
**Why this system uses it:** Email bodies containing PII use envelope encryption — CMK encrypts DEK, DEK encrypts email body (which can be megabytes — well above KMS 4KB limit). SMTP credentials for relay services (SendGrid, SES) stored in Secrets Manager with auto-rotation. DKIM private signing keys stored in Secrets Manager (not in code or environment variables). IRSA role for email service pod → GetSecretValue for SMTP credentials — zero hardcoded credentials.