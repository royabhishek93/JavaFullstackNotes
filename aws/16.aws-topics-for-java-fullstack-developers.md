...
Use this as your transition script after architecture overview:

"Once I present the high-level design, I go deep into operational tradeoffs. I separate high availability from read scaling, then I explain security, cost control, and failure handling with concrete production examples."

ASCII quick map:

~~~text
HA path:      App -> Primary DB -> Sync Standby (failover)
Scale path:   App -> Primary DB + Read Replicas (read offload)
Ops path:     Metrics -> Alarm -> Triage -> Fix -> Postmortem
~~~

Print notes:

- Spoken Q&A structure for senior interview rounds.
- ASCII recall map added for fast revision.
- Positioned as deep-dive file after SEQ-01.

## 15Y Conversational Script (ASCII-Only)

Say this naturally:

"When I discuss AWS at senior level, I separate availability, scalability, security, and operations. I do not mix them, because each has different failure modes and different controls."

"For databases, Multi-AZ is for failover continuity, not read scaling. Read replicas are for throughput and read fan-out. In production I usually use both: Multi-AZ on primary for resilience, replicas for scale."

ASCII interview diagram:

~~~text
Write path:
App -> Primary DB (Multi-AZ standby attached)

Failover path:
Primary down -> AWS promotes standby -> DNS endpoint remains stable

Read scale path:
App read traffic -> Replica-1 / Replica-2 / Replica-3

Ops loop:
Metrics -> Alarm -> On-call -> Mitigation -> RCA -> Preventive action
~~~

15Y signal lines:

- "I choose consistency level per use case, not by default."
- "I quantify tradeoffs: latency, recovery time, and monthly cost impact."
- "I design incident response into the architecture, not after incidents happen."
...
```
    [X] BAD APPROACH - HARDCODED CREDENTIALS [X]
...
```
    ...
    Or use formula:
    Pool size = (CPU cores x 2) + effective_spindle_count
    For CPU-bound: 8 cores x 2 = 16 connections
...
"Now, there's also a formula: pool size = CPU cores x 2. For an 8-core server, that's 16 connections. But this assumes CPU-bound workloads. For I/O-bound workloads - where queries wait on disk or network - you can go higher, maybe 20-30. The key is: don't over-provision. More connections doesn't always mean better performance."
...
```
    ...
    +--------------------------+
    | Cache entry expires      | [OK] Simple
    | after fixed time         | [OK] No manual work
    +--------------------------+ [X] Might serve stale data
    
    Strategy 2: Write-Through
    +--------------------------+
    | Update DB                | [OK] Always fresh
    |     v                    | [X] Extra write overhead
    | Update Cache             | [X] Both must succeed
    +--------------------------+
    
    Strategy 3: Cache Invalidation
    +--------------------------+
    | Update DB                | [OK] Simple
    |     v                    | [OK] Cache refreshes on read
    | Delete from Cache        | [X] Next read is slow (miss)
    +--------------------------+
...
```
    [X] WRONG ORDER - CACHE FIRST
    
    1. Update cache (new price: $100)
    2. Update database
       v
    [WARN] Database update FAILS!
    [WARN] Cache has $100, DB has $120
    [WARN] User sees wrong price!
    
    
    [OK] CORRECT ORDER - DATABASE FIRST
...
```
    ...
    | [OK] Automated smoke tests             |
    +------------+------------------------+
                 |
    +------------v------------------------+
    | STEP 6: Deploy to STAGING           |
    | +- Same process as DEV              |
    | [OK] Run full integration test suite   |
...
```
    ...
    +- Error rate: Blue 0.1%, Green 0.1% [OK]
    +- Latency P95: Blue 200ms, Green 195ms [OK]
    +- CPU usage: Blue 60%, Green 58% [OK]
    +- Business metrics: Orders processing [OK]
...
```
    ...
    Monitor for 10 minutes
         v
    [WARN] ERROR: Green error rate = 5%
    [WARN] Blue error rate = 0.1%
    [WARN] Something wrong with Green!
         v
    IMMEDIATE ROLLBACK:
...
```
    ...
    1. SELECT * FROM orders 
       ...
       +- No index on (user_id, status)! [CRITICAL]
...
[X] "I just click in the console"
[OK] "I use Infrastructure as Code with CloudFormation"

[X] "I put credentials in environment variables"
[OK] "I use IAM roles attached to EC2 instances"

[X] "I haven't debugged production issues"
[OK] "Let me tell you about an incident I resolved..."

[X] "I only know EC2"
[OK] "I use EC2, RDS, ElastiCache, S3, and CloudWatch together"
...
**Remember:**
[OK] **Speak naturally** - like you're explaining to a colleague
[OK] **Draw diagrams** - visual explanations are clearer
[OK] **Use real examples** - shows actual experience
[OK] **Quantify results** - numbers prove impact
[OK] **Admit what you don't know** - and show how you'd learn it
...
"Then I drill into EC2. I see 50 m5.2xlarge instances running 24/7, tagged as 'dev-environment'. Each m5.2xlarge costs about $0.38/hour, so 50 instances x 24 hours x 30 days x $0.38 = $13,680 per month. That's the culprit right there."
...
```
    ...
    +- DR region promoted
    +- Order #12345 exists [OK]
    +- Order #12346 does NOT exist [X]
...
```
    ...
    +- EC2: 10 instances x $500/month = $5,000
    +- RDS: db.r5.2xlarge = $1,500
    +- S3: $500
    +- Total: $7,000/month
    
    DR Region (US-WEST-2):
    +- EC2: 3 instances x $500/month = $1,500
...
```
    ...
    |                                     |
    | 2024-01-15 03:25:10                 |
    | RunInstances (50 x c5.24xlarge)    |
    | Region: us-east-1                   |
    | [X] Cryptocurrency mining!            |
    |                                     |
    | 2024-01-15 03:27:33                 |
    | CreateAccessKey (for backdoor user) |
    | [X] Attacker created another key!     |
    +-------------------------------------+
...
```
    ...
    [OK] 1. Deactivate original compromised key
    
    [OK] 2. Delete backdoor user "new-user-backdoor"
       +- Removes their access keys automatically
       +- Terminates their session
    
    [OK] 3. Terminate all 50 mining instances
       +- Filter by: User = "new-user-backdoor"
       +- Terminate all immediately
    
    [OK] 4. Revoke all sessions for that user
       +- AWS STS -> RevokeSession API
       +- Kicks out active sessions
    
    [OK] 5. Check for other compromised resources
    ...
    [OK] 6. Enable AWS GuardDuty (if not already)
...
```
    ...
    |    SELECT * FROM order_items        |  50ms x 10
...
```
    ...
    Target: <500ms [OK] ACHIEVED
    
    Improvement: 75.75% reduction
    
    No schema changes [OK]
    No additional servers [OK]
    Zero cost increase [OK]
```
