...
### 1. Abstract Piece Class (~5 mins) - Strategy Pattern
```java
...
### 2. ONE Concrete Piece - Knight (~3 mins)
```java
...
### 3. Game Class with Check Detection (~12 mins)
```java
public class Game {
    private Board board;
    ...
    private void undoSimulation(Position from, Position to, 
...
- "King moves 1 square any direction, Queen = Rook + Bishop"
- "Rook moves horizontally/vertically, Bishop moves diagonally"
- "Pawn: forward 1 (or 2 on first move), captures diagonally"
- "Position is immutable value object with row, col (0-7)"
- "Board is 8x8 Piece[][] array with helper methods"
- "Checkmate: King in check + no valid moves available"
- "Color enum: WHITE, BLACK with opposite() method"
