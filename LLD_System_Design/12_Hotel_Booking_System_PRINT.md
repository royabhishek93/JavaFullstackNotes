...
### 1. Booking Class with Overlap Logic (~5 mins) - KEY DIFFERENCE
```java
public class Booking {
    ...
    // CRITICAL: Time overlap detection
...
### 2. Room with Time-based Availability (~8 mins)
```java
public class Room {
...
- "90% same as Parking Lot - just add TIME dimension"
- "Hotel class manages List<Room>, similar to Level managing spots"
- "Overlap formula: (start1 < end2) AND (end1 > start2)"
- "BookingStatus enum: PENDING, CONFIRMED, CANCELLED, COMPLETED"
- "RoomType enum: SINGLE, DOUBLE, DELUXE, SUITE with base prices"
- "For pricing: PricingStrategy with weekend rates, peak season, dynamic pricing"
- "Thread safety: ReentrantLock on room bookings prevents double booking"
